...
def train(args, train_dataset, model, tokenizer):
    ...
    # f = open(os.path.join(args.output_dir, "model_results.tsv"), "w")
    # params = ["all"] + [name for name, param in model.named_parameters()]
    # full_param_names = []
    # for i in params:
    #     full_param_names.extend([i+".data", i+".grad"])
    # f.write("\t".join(full_param_names) + "\n")
    # f.close()
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.fp16:
                ...
                loss.backward()
            # get_model_weight_gradient_stats(model, args.output_dir)
            tr_loss += loss.item()
